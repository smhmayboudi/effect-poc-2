  ...
  Effect.gen(function* (_) {
    ...
    const supervisor = yield* _(
      Effect.acquireRelease(Supervisor.track, supervisor =>
        pipe(supervisor.value, Effect.flatMap(Fiber.interruptAll))
      )
    );
    ...
    >(
      ...
      pipe(
        Effect.runtime<Effect.Effect.Context<ReturnType<Handlers[number]>>>(),
        Effect.map(runtime =>
          Array.map(
            handlers,
...
export const withExpressServer = <A, E, R>(
  ...
  pipe(
    Express,
    Effect.flatMap(({server}) => f(server))
  );
